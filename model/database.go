...
func setupTables() {
	var t = []struct {
		...
		{"team", `CREATE TABLE team ( teamID varchar(64) NOT NULL, owner varchar(32) NOT NULL, name varchar(64) DEFAULT NULL, rockskey varchar(32) DEFAULT NULL, rockscomm varchar(32) DEFAULT NULL, joinLinkToken varchar(64), telegram bigint signed, vteam int(11) UNSIGNED DEFAULT 0, vrole int(11) UNSIGNED DEFAULT 0, PRIMARY KEY (teamID), KEY fk_team_owner (owner), CONSTRAINT fk_team_owner FOREIGN KEY (owner) REFERENCES agent (gid) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;`},
		...
		{"agentextras", `CREATE TABLE agentextras ( gid varchar(32) NOT NULL, picurl text, VAPIkey varchar(42), UNIQUE KEY gid (gid), CONSTRAINT fk_extra_agent FOREIGN KEY (gid) REFERENCES agent (gid) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;`},
		...
		{"markerattributes", `CREATE TABLE markerattributes (ID varchar(64) NOT NULL, opID varchar(64) NOT NULL, markerID varchar(64) NOT NULL, assignedTo varchar(32), assignedTeam varchar(64), name varchar(32) NOT NULL DEFAULT 'unset', value TEXT, PRIMARY KEY (ID, opID), KEY fk_makerattr_opID (opID), CONSTRAINT fk_markerattr_opID FOREIGN KEY (opID) REFERENCES operation (ID) ON DELETE CASCADE, KEY fk_marker_attr (markerID), CONSTRAINT fk_marker_attr FOREIGN KEY (markerID) REFERENCES marker (ID) ON DELETE CASCADE, KEY fk_agent_markerattr (assignedTo), CONSTRAINT fk_agent_markerattr FOREIGN KEY (assignedTo) REFERENCES agent (gid) ON DELETE CASCADE, KEY fk_team_markerattr (assignedTeam), CONSTRAINT fk_team_markerattr FOREIGN KEY (assignedTeam) REFERENCES team (teamID) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`},
		{"depends", `CREATE TABLE depends (opID varchar(64) NOT NULL, taskID varchar(64) NOT NULL, dependsOn varchar(64), KEY fk_depends_opID (opID), CONSTRAINT fk_depends_opID FOREIGN KEY (opID) REFERENCES operation (ID) ON DELETE CASCADE, KEY key_optask (opID,taskID) ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`},
		{"v", `CREATE TABLE v (enlid char(42) NOT NULL, gid varchar(32) NOT NULL, vlevel int, vpoints int, agent varchar(64), level int, quarantine tinyint not null default 0, active tinyint not null default 0, blacklisted tinyint not null default 0, verified tinyint not null default 0, flagged tinyint not null default 0, banned tinyint not null default 0, cellid varchar(32), telegram varchar(32), telegramID int NOT NULL default 0, startlat float, startlon float, distance int, fetched datetime NOT NULL DEFAULT UTC_TIMESTAMP(), PRIMARY KEY fk_v_gid (gid), CONSTRAINT fk_v_gid FOREIGN KEY (gid) REFERENCES agent (gid) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`},
		{"rocks", `CREATE TABLE rocks (gid varchar(32) NOT NULL, tgid int, agent varchar(64), verified tinyint not null default 0, smurf tinyint not null default 0, fetched datetime NOT NULL DEFAULT UTC_TIMESTAMP(), PRIMARY KEY fk_rocks_gid (gid), CONSTRAINT fk_rocks_gid FOREIGN KEY (gid) REFERENCES agent (gid) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`},
	}
...
func upgradeTables() {
	var upgrades = []struct {
		...
		// {"SELECT DATA_TYPE FROM information_schema.columns WHERE table_schema = Database() AND table_name = 'agent' AND column_name = 'fetched'", "ALTER TABLE agent DROP Vverified, DROP Vblacklisted, DROP Vid, DROP RocksVerified, DROP Vname, DROP rocksname, DROP RAID, DROP level, DROP name, ADD fetched datetime not null default current_timestamp()"},
	}
