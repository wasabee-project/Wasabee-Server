...
func setupTables() {
	var t = []struct {
		...
		{"v", `CREATE TABLE v (enlid varchar(32) NOT NULL, gid varchar(32) NOT NULL, vlevel int, vpoints int, agent varchar(64), level int, quarantine tinyint not null default 0, active tinyint not null default 0, blacklisted tinyint not null default 0, verified tinyint not null default 0, flagged tinyint not null default 0, banned tinyint not null default 0, cellid varchar(32), telegram varchar(32), startlat float, startlon float, distance int, fetched datetime NOT NULL DEFAULT UTC_TIMESTAMP(), PRIMARY KEY fk_v_gid (gid), CONSTRAINT fk_v_gid FOREIGN KEY (gid) REFERENCES agent (gid) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`},
