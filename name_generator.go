...
func GenerateSafeName() (string, error) {
	...
	for rows > 0 {
		...
		err := db.QueryRow("SELECT COUNT(id) FROM documents WHERE id = ?", hex.EncodeToString(databaseID[:])).Scan(&i)
		if err != nil {
			return "", err
		}
		total = i
		err = db.QueryRow("SELECT COUNT(lockey) FROM user WHERE lockey = ?", hex.EncodeToString(databaseID[:])).Scan(&i)
		if err != nil {
			return "", err
		}
		total = total + i
		err = db.QueryRow("SELECT COUNT(teamID) FROM teams WHERE teamID = ?", hex.EncodeToString(databaseID[:])).Scan(&i)
		if err != nil {
			return "", err
		}
		total = total + i
